...
# ─── API keys for the public predict endpoint ──────────────────────────────
#
# Keys are scoped to a single deployment. The plaintext is generated by
# secrets.token_urlsafe(32) (~256 bits), prefixed with "mlops_" so callers
# can identify what kind of secret it is, and stored as a SHA-256 hex digest
# so a DB leak doesn't expose usable credentials. We only ever return the
# plaintext ONCE -- at creation time. After that, only the 12-char prefix is
# shown in the UI.


def _generate_api_key() -> tuple[str, str, str]:
    """Returns (plaintext, sha256_hex, display_prefix)."""
    plaintext = f"mlops_{secrets.token_urlsafe(32)}"
    key_hash = hashlib.sha256(plaintext.encode()).hexdigest()
    prefix = plaintext[:12]
    return plaintext, key_hash, prefix


async def _resolve_owned_deployment(
    deployment_id: str, current_user: User, db: AsyncSession
) -> Deployment:
    """Fetch a deployment iff it belongs to the current user's project. 404
    on miss (don't leak that the id exists but isn't theirs)."""
    result = await db.execute(
        select(Deployment)
        .join(Project, Deployment.project_id == Project.id)
        .where(Deployment.id == deployment_id, Project.user_id == current_user.id)
...
@router.post("/{deployment_id}/api-keys", response_model=ApiKeyCreatedResponse)
async def create_api_key(
    ...
    dep = await _resolve_owned_deployment(deployment_id, current_user, db)
    plaintext, key_hash, prefix = _generate_api_key()
    api_key = DeploymentApiKey(
        deployment_id=dep.id,
        key_hash=key_hash,
        prefix=prefix,
    ...
    # ApiKeyCreatedResponse is the ONLY response shape that includes the
    # plaintext -- save it now, you'll never see it again.
...
@router.get("/{deployment_id}/api-keys", response_model=list[ApiKeyResponse])
async def list_api_keys(
    ...
    dep = await _resolve_owned_deployment(deployment_id, current_user, db)
    result = await db.execute(
        select(DeploymentApiKey)
        .where(DeploymentApiKey.deployment_id == dep.id)
        .order_by(DeploymentApiKey.created_at.desc())
    )
    return result.scalars().all()


@router.delete("/{deployment_id}/api-keys/{key_id}")
async def revoke_api_key(
    ...
    result = await db.execute(
        select(DeploymentApiKey).where(
            DeploymentApiKey.id == key_id,
            DeploymentApiKey.deployment_id == dep.id,
        )
    )
