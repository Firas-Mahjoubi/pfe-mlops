...
| `05_churn_end_to_end.ipynb` | **The showcase** — a long, realistic workflow: synthetic telecom-churn dataset (6,000 rows, missing values, categorical features), EDA plots, feature engineering, `ColumnTransformer` preprocessing, 3-model comparison, manual tuning, confusion matrix / ROC / feature importances, and a ready-to-paste prediction payload. | Converted to `05_churn_end_to_end.py`, no warnings. | **One execution → 6 runs across 3 model families** (LogisticRegression, RandomForest ×4, GradientBoosting), grouped and ranked in Experiments. The final RandomForest is a full Pipeline (positional column selectors) — registrable **and deployable**; the last cell prints the exact payload for the prediction tester. |
